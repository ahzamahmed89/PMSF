...
export default function ActivityCard({ activity, onActivityUpdate, onMediaAdd, onMediaRemove }) {
  ...
  // useEffect: When hasStream becomes true and showCamera is true, connect stream
  useEffect(() => {
    if (hasStream && showCamera && streamRef.current && videoRef.current) {
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      // Tell React that we have a stream, which will trigger useEffect
      setHasStream(true);
      
      // NOW show camera UI - this will trigger useEffect which connects stream
      setShowCamera(true);
